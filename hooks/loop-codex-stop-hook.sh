...
2. **先诊断，再谈换结构**：读数在噪声带里不代表"这条路走到头了"，只代表**改动没有
   碰到真正限制它的东西**。这时直接做结构性改动，是在缺诊断的基础上换一个更大的猜测。
   \`bash bench/headroom.sh\` 的「历次尝试」表会告诉你哪些分支已经试过、动没动；
   净变化接近 0 的分支，再挑它必须说明这次改的是前几次没碰到的东西。
   **本循环还没有 profiler 产物就先取证据**：\`bash bench/profile_case.sh <case_id>\`，
   看 PipeUtilization 哪条流水在限速；再独立核一遍搬运字节数 ÷ 实测耗时，和 baseline 比。
   拿到证据后再选分支——按分支表的 \`填缺口需\`（只补上缺口要提速几倍）挑，
   **不要按 \`剩余分\` 挑**：那是够不到的上界，按它排会选中最难的那条。
...
3. **先看这次改动整体值多少**：把退步分支的丢分和其余分支的得分放在一起算
   \`mean(perf_score)\` 的净变化。净变化为正就**不要回滚**——记下副作用、单独修，
   把好的部分留住；实测过一次反例：某改动 3 个 case 变快、2 个变慢，几何平均 0.9993×，
   回滚正确，但换成整体 +30%、只有一条窄分支退 5% 的情形，回滚就是把收益扔掉。
4. 净变化为负或持平才回滚。**不要靠新的优化去盖过退步**：那样每次测量都建立在
...
**换结构分两级。先跑 \`bash bench/headroom.sh\`，看 device kernel 分支表再决定用哪级。**

---

### 第一级：分支级修复（默认从这里开始）

分支表列出每条 device kernel 分支覆盖多少 case、平均加速比多少、\`剩余分\`（到物理上限
还差多少分，是够不到的上界）和 \`需提速\`（到那个上界要快多少倍）。
**选一条分支做修复，不要整体重写。选哪条由你判断，但必须在报告里写出理由。**

选之前先看 headroom 的两张表：
- 分支表的 \`填缺口需\`——只补上缺口要提速几倍。**行序不是建议**，它不知道你试过什么。
- 「历次尝试」表——每条分支在各轮的实测加速比。**净变化接近 0 不是"这条分支难"，
  是"那些轮的改动没碰到真正卡住它的东西"**；为负表示在退步。

**读数没动，不是换分支的理由。** 换一条只会把最便宜的那条先放弃掉（它通常最先被试），
而且往往每条都一样平——那时换来换去毫无意义。读数没动只说明一件事：**缺诊断**。
这时下一步是取证据（profiler / 逐 case 计时 / 数据量核算），不是换分支，也不是加大改动。

1. 记下锚分支当前的 case 数、平均加速比、\`mean(perf_score)\`——这是本级的基准。
2. 判断还有哪些分支**共享同一个根因**。表里平均加速比同样偏低、且诊断指向同一处
   （同样的归约方式 / 同样的 UB 分块 / 同一族的 dtype 变体）的，一并改——
   但要在报告里写明共享的根因是什么，并**逐条列出每个被改分支改动前后的
   \`mean(perf_score)\`**。说不清共享根因就只改锚分支。
3. **不要让任何分支的 \`mean(perf_score)\` 退步。** 已达标（\`perf_score >= 0.8\`）的 case 是
   已经拿到手的分数——整体重写之所以代价大，就是每次都把这些一起推翻。
...
> 不要因为「想不出新点子」升级。也不要跳过第一级——整体重写要重新面对全部 case，
> 把已经调好的分支一起推翻，代价比它看起来大得多。
