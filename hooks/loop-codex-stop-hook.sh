...
**只改可回收分最高的那一条，其余分支一行都不要动。**

1. 记下该分支当前的 case 数、平均加速比、\`mean(perf_score)\`——这是本级的基准。
2. 只在这条分支的代码路径上做结构性改动。已经超过 baseline 的分支（表里平均加速比 > 1×）
   是资产，动它们只会把已有的分数赔进去。
3. 改完跑正式评测，比较该分支的 \`mean(perf_score)\`。

**升级条件**：该分支已试过 **2 个**结构上不同的子方案，且最好的一次相对本级基准
