...
分支表列出每条 device kernel 分支覆盖多少 case、平均加速比多少、追平 baseline 值多少分。
**以可回收分最高的那条为锚，做分支级修复，不要整体重写。**
...
3. **排序看「打到上限可回收」那一列，不要看「追平可回收」。** 追平只是 \`perf_score=0.5\`，
   而目标是 **0.8**——一条平均加速比已经 > 1× 的分支，里面照样可能有 case 远未达标。
   「打到上限可回收」已经把距目标的差距算进去了。
4. **不要让任何分支的 \`mean(perf_score)\` 退步。** 已达标（\`perf_score >= 0.8\`）的 case 是
   已经拿到手的分数——整体重写之所以代价大，就是每次都把这些一起推翻。
   但「已超过 baseline」不等于「已达标」，别把这类分支当成不能碰。
5. 改完跑正式评测，逐分支比较 \`mean(perf_score)\`，并确认没有分支退步。
